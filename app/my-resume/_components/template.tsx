        ...
        <div className="w-[90%] bg-[rgba(255,255,255,0.2)] min-h-80 mx-auto mb-40 rounded-[15px] border border-solid border-white grid grid-cols-2 md:grid-cols-4 p-[4%] sm:p-[3%] gap-4 sm:gap-6 md:gap-x-12 md:gap-y-10">
            ...
            {showIncompleteModal && (
                <div className="fixed top-0 left-0 w-screen h-screen bg-[rgba(0,0,0,0.7)] flex justify-center items-center z-10000" onClick={() => setShowIncompleteModal(false)}>
                    ...
                    {/* 🌟 จุดแนะนำเพิ่มเติม: ปรับขนาด Modal แจ้งเตือนโปรไฟล์ไม่สมบูรณ์ให้ยืดหยุ่น ไม่ล้นขอบจอมือถือแคบๆ ด้วย w-[90%] sm:w-md */}
                    <div className={`bg-[#3A1C63] p-6 sm:p-9 rounded-[15px] text-center w-[90%] sm:w-md h-auto shadow-[0_10px_30px_rgba(0,0,0,0.3)] ${styles.incompleteBoxAni}`} onClick={(e) => e.stopPropagation()}>
                        ...
                        <p className="mb-4 text-sm sm:text-base">You haven't finished setting up your profile. Would you like to continue anyway?</p>
            
                        <button 
                            className="border-none text-white font-inherit bg-[#5F28CD] py-3.5 sm:py-4 px-8 sm:px-13 rounded-[50px] text-[1rem] font-semibold transition-all duration-150 ease-in cursor-pointer hover:bg-[#4716a9] w-full sm:w-auto" 
